...
# Movement tuning (exported so you can tweak live via Inspector while embedded-playing)
@export var acceleration: float = 400.0
@export var deceleration: float = 600.0
@export var max_speed: float = 120.0
@export var max_reverse_speed: float = 70.0
@export var turn_speed: float = 1.5  # radians/sec, player-controlled turning

# Collision impact tuning
...
func _physics_process(delta: float) -> void:
	...
	# Resync speed with what actually happened post-collision
	speed = velocity.dot(forward)

	# Spin the player when hitting a surface at an angle
